    ...
    def __init__(
        ...
        if use_adahedge:
            ...
            if restricted_exploration:
                ...
                allocation_A = np.concatenate([self.constraints + f*w_t_norm*np.ones_like(self.constraints) , -eye, simplex, -simplex], axis=0)
...
def run_exploration_experiment(bandit, explorer, A, b):
    ...
    while not done and t<100000:
        ...
        if policy is not None:
            simple_regret.append(A.dot(optimal_policy-policy))
            diff = A.dot(policy) - b
            #print(diff)
            if (diff > 0).sum() > 0:
...
if __name__ == "__main__":
    ...
    with open('CGE_WLag_easy.txt', 'w') as f:
