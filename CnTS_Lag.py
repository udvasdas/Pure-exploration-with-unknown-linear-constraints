...
class Explorer:
    ...
    def stopping_criterion(self, vertex, f_t ,w_t_norm):
        """
        ...
        #print(beta)
        #print(best_l.dot(self.b-opt_const.dot(vertex)))
        #print((best_l.sum())*(f_t*np.sqrt(w_t_norm)+1))
        return self.t*game_value + best_l.dot(self.b-opt_const.dot(vertex)) >  beta + (best_l.sum())*(f_t*np.sqrt(w_t_norm)+1)
    ...
    def update(self, arm, reward, cost):     # Update empirical estimates
        ...
        self.means[arm] = self.means[arm] + (1 / self.n_pulls[arm]) * (reward - self.means[arm])
        self.constraints.T[arm] = ((self.n_pulls[arm]-1)*self.constraints.T[arm] + cost)/self.n_pulls[arm] 
        #self.means = np.linalg.inv(self.gram_mat).dot(reward*np.eye(self.constraints.shape[1])[arm])
        #for i in range(self.constraints.shape[0]):
            #self.constraints[i] = np.linalg.inv(self.gram_mat).dot(cost[i]*np.eye(self.constraints.shape[1])[arm])
...
class TnS(Explorer):
    ...
    def act(self):
        ...
        if results["success"] == True:   
            #optimal_policy = optimal_policy.round(3)
            #optimal_policy = optimal_policy.round(2)
            ...
            f_t = 1 + np.sqrt((1/2) * np.log(4/self.delta)
                                        + (len(self.means)/4) * np.log(1 + self.t/len(self.means)))
            opt_constraint = self.constraints - f_t*np.sqrt(w_t_norm)
            #print(self.t)
            #print(1)
            ...
            stop = self.stopping_criterion(optimal_policy,f_t,w_t_norm)
            #print(stop)
...
if __name__ == "__main__":
    ...
    with open('CnTS_Lag_1_easy.txt', 'w') as f:
